...
### 14. image() does NOT support source-rect (9-arg form)

p5.js `image()` only accepts `image(img, x, y)` or `image(img, x, y, w, h)`. There is NO 9-argument source-rect overload like HTML canvas `drawImage()`. Passing 9 args silently no-ops — nothing renders, no error.

```javascript
// DOES NOT WORK — silently fails, nothing drawn
image(spriteSheet, dx, dy, dw, dh, sx, sy, sw, sh);

// WORKS — use drawingContext.drawImage() with p5.Image.canvas
drawingContext.drawImage(spriteSheet.canvas,
  sx, sy, sw, sh,    // source rect
  dx, dy, dw, dh);   // dest rect

// The .canvas property exposes the underlying HTMLCanvasElement.
// p5's transform stack (push/translate/scale) still applies because
// drawImage honors the current drawingContext transform matrix.
```

This is essential for **sprite sheet rendering** — drawing sub-regions of a larger image.

### 15. Variable shadowing of p5.js built-in functions

Never name a variable or function parameter `text`, `line`, `fill`, `stroke`, `rect`, `ellipse`, `image`, `scale`, `translate`, `color`, or any other p5.js global function name. Doing so shadows the global and causes `TypeError: X is not a function` at runtime.

```javascript
// BAD — 'text' parameter shadows p5.js text() function
function drawLabel(cx, cy, text) {
  text(text, cx, cy);  // TypeError: text is not a function
}

// GOOD — use a different name
function drawLabel(cx, cy, label) {
  text(label, cx, cy);
}
```

This is especially insidious because the TypeError kills the `draw()` loop silently (see #16 below).

### 16. Uncaught exceptions in draw() kill the animation silently

p5.js catches exceptions thrown inside `draw()` and **stops calling `draw()` again**. There is no console error, no crash message — the canvas just freezes on frame 1. This is the #1 cause of "my sketch shows one frame then stops."

**Diagnosis — add a frame counter:**
...
Check via browser console: if `drawCallCount` is stuck at 1, something in `draw()` is throwing. **Isolate by calling sub-functions in try/catch:**

```javascript
const errors = [];
try { drawBackground(); } catch(e) { errors.push('drawBackground: ' + e.message); }
try { drawCharacters(); } catch(e) { errors.push('drawCharacters: ' + e.message); }
// Check errors array — the one that fires is your culprit
```

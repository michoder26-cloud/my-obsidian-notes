...
### Ask for a reference image before building anything elaborate

When the user asks for a UI/dashboard/interface — especially one inspired by a specific style (pixel art, game UI, retro, etc.) — ask for a reference screenshot or URL FIRST. Do not build a complex design (canvas spritesheets, walking animations, multi-tile office layouts) without seeing exactly what the user has in mind. A single reference image saves you hours of wrong-direction work.

### Err on the side of simplicity until proven otherwise

If the user's request is ambiguous between "elaborate themed design" and "clean functional UI", default to the simpler option. Build a clean, functional, light-themed HTML page first and let the user direct you toward more complexity. It is much faster to add polish to a working simple page than to tear down a complex canvas-based implementation the user rejects.

### When the user says "delete it and start over", do it cleanly

Don't try to salvage parts of a rejected approach. If the user tells you to remove something and start from a new reference, remove it fully and build fresh from the new direction. Partial salvage attempts waste cycles and frustrate the user — the new reference is the single source of truth for the replacement.

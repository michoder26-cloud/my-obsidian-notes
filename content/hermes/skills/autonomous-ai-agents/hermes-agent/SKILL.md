...
Connect the Hermes Desktop App to a remote VPS backend:

```bash
# Set credentials (recommended - edits config.yaml directly)
hermes config set basic_auth.username admin
hermes config set basic_auth.password "$(openssl rand -base64 12)"
hermes config set basic_auth.secret "$(openssl rand -base64 32)"

# Start dashboard on reachable address
hermes dashboard --no-open --host 0.0.0.0 --port 9119

# On Desktop App: connect to http://YOUR_VPS_IP:9119 with the username/password above
```

**Pitfalls:**
- Using `&` in foreground terminal → blocked. Use `terminal(background=true)` instead.
- `.env` file may be protected — prefer `hermes config set` over manual editing.
- After setting credentials, restart dashboard: `pkill -f "hermes dashboard"; sleep 1; hermes dashboard ...`
...
### Spawned Agent → Web Dashboard

When spawning agents via `delegate_task`, you can make them visible on
a companion web dashboard using a shared JSON file + HTTP API. See
`skill_view(name="hermes-agent", file_path="references/spawned-agent-dashboard.md")`.
...
- **Platform-specific issues** (`Alt+Enter` newline, WinError 10106, UTF-8 BOM config, test suite, line endings): see the dedicated **Windows-Specific Quirks** section above.
- **VPS service down**: If a remote service (dashboard, HTTP server, etc.) is unreachable, follow the VPS service recovery guide: `skill_view(name="hermes-agent", file_path="references/vps-service-recovery.md")`. Always check: browser error → `ss -tlnp` for port → `ps aux` for process → restart.
